# https://github.com/elixir-lang/ecto/blob/master/lib/ecto/changeset.ex
defmodule Amnesia.Changeset do
  @moduledoc """
  ...
  * `filters`     - Filters (as a map `%{field => value}`) to narrow the scope of update/delete queries
  ...
  """
  @spec change(model | t, %{atom => term} | [Keyword.t]) :: t
    ...
    {key,
      case cast_field(param_key, type, params) do
        ...
        :missing ->
          {changes, error_on_nil(kind, key, current, errors)}
        :invalid ->
          {changes, [{key, "ist ungültig"}|errors]}
      end}
  end
  ...
  defp cast_field(param_key, type, params) do
    case Map.fetch(params, param_key) do
      {:ok, value} ->
        case Amnesia.Type.cast(type, value) do
          {:ok, value} -> {:ok, value}
          :error       -> :invalid
        end
      :error ->
  ...
  defp error_on_nil(:required, key, nil, errors),
    do: [{key, "darf nicht leer sein"}|errors]
